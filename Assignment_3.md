...
sambamba view -h -c -S -F "[XS] == null and not unmapped and not duplicate" H3K27me3_iPSC_SRA60_subset_1_LEN_25_aligned.sam -o H3K27me3_iPSC_SRA60_subset_1_LEN_25_mapped.sam
...
sambamba view -h -c -F "[XS] == null and not unmapped and not duplicate"  paired_end_bowtie_alignment.bam -o paired_end_uniq_mapping_reads.bam
#output 2686235

#?# Use sambamba view to get the number of uniquely mapped reads for the SE alignment, type the command you used below: - 0.5 pt 
sambamba view -h -c -F "[XS] == null and not unmapped and not duplicate"  single_end_bowtie_alignment.bam -o single_end_uniq_mapping_reads.bam
# output 1325510
```
...
# Paired end reads refer to the fact that a given DNA molecule is sequenced twice, 
# once in the 5' -> 3' direction and once in the 3' -> 5' direction. When it comes
# to alignment, both of the transcripts are aligned to the reference genome, and 
# then are combine together using some algorithmic approach outline by the alignment
# tool of choice. On the other hand, single end reads refer to a DNA fragment being
# mapped in only one direction, as opposed to both directions. In the SE alignment,
# there is no step to combine fragments after aligning to the reference as you are
# only working with a single fragment.
...
#Between the two, the 25bp paired end reads has more uniquely mapping reads in
#comparison to the 50bp single end reads. This is because paired end read alignments 
#also allow for mapping to more regions in the reference, such as repetitive regions
#which are typically missed when only performing a single end read alignement.
```
